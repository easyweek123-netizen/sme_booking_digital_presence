...
import type { OnlineInput } from '@bookeasy/shared';

interface OnlinePickerProps {
  value: OnlineInput | null;
  onChange: (next: OnlineInput | null) => void;
}

export function OnlinePicker({ value, onChange }: OnlinePickerProps) {
  const { data: calendarStatus, isLoading } = useGetCalendarStatusQuery();
  const connectedId =
    calendarStatus?.connected && calendarStatus.calendarId ? calendarStatus.calendarId : null;

  useEffect(() => {
    if (connectedId == null) return;
    if (value?.calendarId !== connectedId) onChange({ calendarId: connectedId });
  }, [connectedId, value?.calendarId, onChange]);
    ...
    <VStack spacing={4} align="stretch">
      <Box bg="surface.card" border="1px solid" borderColor="border.subtle" borderRadius="md" p={4}>
        ...
        <HStack spacing={1.5} mt={2}>
          ...
          <Text fontSize="sm" color="feedback.success.fg">
            Calendar will be linked when you save.
          </Text>
        </HStack>
      </Box>
    </VStack>
