...
export function Suggestions({ suggestions, onSelect }: SuggestionsProps) {
  return (
    <Wrap spacing={2} mt={3}>
      {suggestions.map((suggestion: Suggestion) => {
        const isSelected = suggestion.isSelected === true;
        const isSkip = suggestion.variant === 'skip';
        return (
          <WrapItem key={suggestion.value}>
            <Button
              size="sm"
              variant={isSelected ? 'solid' : isSkip ? 'ghost' : 'outline'}
              colorScheme={isSelected ? 'brand' : undefined}
              borderRadius="full"
              fontWeight="500"
              color={isSelected ? undefined : isSkip ? 'text.muted' : 'text.primary'}
              borderColor={isSelected ? undefined : isSkip ? 'transparent' : 'border.subtle'}
              onClick={() => onSelect(suggestion.value, suggestion.label)}
              _hover={
                isSelected
                  ? {}
                  : {
                      borderColor: isSkip ? 'transparent' : 'brand.500',
                      bg: isSkip ? 'surface.alt' : 'brand.50',
                      color: isSkip ? 'text.primary' : 'brand.600',
                    }
              }
            >
              {suggestion.icon && <Text mr={1}>{suggestion.icon}</Text>}
