...
export function useOnboardingSubmit({ state, onComplete, onError }: Options) {
  const [createBusiness, { isLoading: isSubmitting, isError }] = useCreateBusinessMutation();

  const handleAuthSuccess = useCallback(async (_user: User) => {
    try {
  ...
  }, [state, createBusiness, onComplete, onError]);
